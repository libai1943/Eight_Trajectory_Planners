...
s.t. OBCA_DISTANCE {i in {1..nfe}}:
-(lf+lw)*mu[i,1] - lr*mu[i,2] - 0.5*lb*mu[i,3] - 0.5*lb*mu[i,4] + sum {j in {1..ngrids}} (Obstacle[j,1]*x[i] + Obstacle[j,2]*y[i] + Obstacle[j,3])*lambda[i,j] >= dmin;

s.t. OBCA_DUAL_X {i in {1..nfe}}:
mu[i,1] - mu[i,2] + cos(theta[i]) * sum {j in {1..ngrids}} Obstacle[j,1]*lambda[i,j] + sin(theta[i]) * sum {j in {1..ngrids}} Obstacle[j,2]*lambda[i,j] = 0;

s.t. OBCA_DUAL_Y {i in {1..nfe}}:
mu[i,3] - mu[i,4] - sin(theta[i]) * sum {j in {1..ngrids}} Obstacle[j,1]*lambda[i,j] + cos(theta[i]) * sum {j in {1..ngrids}} Obstacle[j,2]*lambda[i,j] = 0;

s.t. OBCA_DUAL_NORM {i in {1..nfe}}:
